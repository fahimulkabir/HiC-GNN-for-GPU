...
def load_input(input, features, device=None):
    ...
    truth = torch.tensor(adj_mat, dtype=torch.double, device=device)
    ...
    node_attr = torch.tensor(features, dtype=torch.float, device=device)
    ...
    # Build SparseTensor on correct device
    adj = SparseTensor(
...
def cont2dist(adj, factor):
    ...
    maxv = torch.max(torch.nan_to_num(dist, posinf=0))
    dist = torch.nan_to_num(dist, posinf=maxv)
    return dist / maxv


def domain_alignment(list1, list2, embeddings1, embeddings2):
    # CPU by design (uses scipy) — do NOT change
